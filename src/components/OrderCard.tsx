...
import { Icon, Button, CircleTip, Translate } from "@biqpod/app/ui/components";
    ...
    if (compact) {
      return (
        ...
        >
          {/* Status Icon */}
          <div
            className={tw("flex-shrink-0 p-2 rounded-full", statusInfo.color)}
          >
          ...
          <div className="flex-1 min-w-0">
            <h4 className="font-medium text-sm truncate">
              Order #{order.id?.slice(-8) || <Translate content="Unknown" />}
            </h4>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              >
                <Translate content={statusInfo.labelKey} />
              </span>
              {order.totalPrice && (
      ...
      >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold text-lg">
                  Order #
                  {order.id?.slice(-8) || <Translate content="Unknown" />}
                </h3>
                <p className="text-[--biqpod-text-secondary] text-sm">
                  <Translate content="Placed on" />{" "}
                  {formatDate(order.createdAt)}
                </p>
              </div>
            </div>
        ...
        <div className="p-4">
          ...
          {(order.client || order.customer) && (
            <div className="mb-4">
              <h4 className="mb-2 font-medium text-sm">
                <Translate content="Customer Information" />
              </h4>
              <div className="bg-[--biqpod-gray-opacity] p-3 rounded-lg">
                {order.client ? (
                  ...
                  <p className="font-medium">
                    <Translate content="Customer ID:" /> {order.customer}
                  </p>
                ) : null}
              </div>
            </div>
          ...
          {showProducts && productCount > 0 && (
            <div className="mb-4">
              <h4 className="mb-2 font-medium text-sm">
                <Translate content="Products" /> ({productCount}{" "}
              ...
              <div className="space-y-2">
                    ...
                    >
                      ...
                      {details?.price && (
                        <span className="font-semibold text-sm">
                          $
                          {(
                            (details.price || 0) * (details.count || 1)
                          ).toFixed(2)}
                        </span>
                      )}
                    </div>
                ...
                {productCount > 3 && (
                  <p className="text-[--biqpod-text-secondary] text-xs text-center">
                    +{productCount - 3} <Translate content="more items" />
                  </p>
                )}
              </div>
            </div>
              ...
              {order.discountAmount && (
                <div className="flex justify-between items-center mt-1 text-sm">
                  <span className="text-green-600">
                    <Translate content="Discount Applied" />
                  </span>
                  <span className="text-green-600">
          ...
          {order.delivery && order.status === "delivery" && (
            <div className="mb-4">
              <h4 className="mb-2 font-medium text-sm">
                <Translate content="Delivery Agent" />
              </h4>
              <div className="bg-[--biqpod-gray-opacity] p-3 rounded-lg">
                <p className="text-sm">
                  <Translate content="Agent ID:" />{" "}
                  {order.delivery.agentId || <Translate content="Assigned" />}
                </p>
                <p className="text-[--biqpod-text-secondary] text-xs">
                  <Translate content="Assigned on" />{" "}
          ...
          {(order.status === "completed" || order.status === "done") && (
            <div className="mt-4 pt-4 border-[--biqpod-border] border-t">
              <h4 className="mb-3 font-medium text-sm">
                <Translate content="Order Timeline" />
              </h4>
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-3 text-sm">
                  ...
                  <span className="text-[--biqpod-text-secondary]">
                    <Translate content="Processing started" />
                  </span>
                </div>
                <div className="flex items-center gap-3 text-sm">
                  <div className="bg-purple-500 rounded-full w-2 h-2"></div>
                  <span className="text-[--biqpod-text-secondary]">
                    <Translate content="Out for delivery" />
                  </span>
                </div>
                <div className="flex items-center gap-3 text-sm">
                  <div className="bg-green-500 rounded-full w-2 h-2"></div>
                  <span className="text-[--biqpod-text-secondary]">
                    <Translate content="Delivered -" />{" "}
